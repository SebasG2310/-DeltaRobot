...
def delta_calcInverse(x0, y0, z0):
    ...
    if status == 0:
        status, theta2 = delta_calcAngleYZ(x0*math.cos(2*pi/3) + y0*math.sin(2*pi/3), y0*math.cos(2*pi/3) - x0*math.sin(2*pi/3), z0)  # Rotate coords to +120 deg
    if status == 0:
        status, theta3 = delta_calcAngleYZ(x0*math.cos(2*pi/3) - y0*math.sin(2*pi/3), y0*math.cos(2*pi/3) + x0*math.sin(2*pi/3), z0)  # Rotate coords to -120 deg
    return status, theta1, theta2, theta3

# Example usage
if __name__ == "__main__":
    x0 = 0  # Example value for x0
    y0 = 0  # Example value for y0
    z0 = 180  # Example value for z0
